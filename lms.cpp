...
enum BookingType {INSTANT, RESERVATION};
...
void User::removeFromCurrent(string bookID){
    for(auto i: this->account.current){
        ...
        if(booking.ID == bookID){
            ...
            assert(booking.bookingType == BookingType::RESERVATION); //will use this only for reservation
...
void loadFromCSV(const string& filename) {
    ...
    while (getline(file, line)) {
        ...
        if (section == "[Books]") {
            ...
            // Read the rest of the line for the reservation queue
